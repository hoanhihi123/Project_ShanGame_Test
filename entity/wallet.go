...
// convert JSON to object Wallet
// payload is a JSON
// []byte(payload) : convert JSON to byte type
func ParseWallet(payload string) (Wallet, error) {
	w := Wallet{}
	err := json.Unmarshal([]byte(payload), &w) // convert JSON to object Wallet
	return w, err
}

func ReadWalletUsers(
	ctx context.Context,
	nk runtime.NakamaModule,
	logger runtime.Logger,
	userIds ...string) ([]Wallet, error) {
...
func ReadWalletUser(ctx context.Context,
	nk runtime.NakamaModule,
	logger runtime.Logger,
	userId string) (Wallet, error) {
	// logger.Info("Read wauserIds %v", nk, ctx, userIds)
	account, err := nk.AccountGetId(ctx, userId) // AccountGetId : api to get info account
	if err != nil {
